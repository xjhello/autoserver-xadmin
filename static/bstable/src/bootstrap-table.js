...
($ => {
  ...
  class BootstrapTable {
        ...
        if (this.options.sortClass !== undefined) {
          ...
          timeoutId = setTimeout(() => {
            ...
            const index = this.$header.find(Utils.sprintf('[data-field="%s"]',
              this.options.sortName).index_bak() + 1)
    ...
    onSort ({type, currentTarget}) {
      ...
      const $this_ = this.$header.find('th').eq($this.index_bak())
    ...
    initBody (fixedScroll) {
      ...
      this.$body.find('> tr[data-index] > td').off('click dblclick').on('click dblclick', ({currentTarget, type, target}) => {
        ...
        const $tr = $td.parent()
        const item = this.data[$tr.data('index_bak.html')]
      ...
      this.$body.find('> tr[data-index] > td > .detail-icon').off('click').on('click', e => {
        ...
        const index = $tr.data('index_bak.html')
      ...
      this.$selectItem.off('click').on('click', e => {
        ...
        const $this = $(e.currentTarget)
        this.check_($this.prop('checked'), $this.data('index_bak.html'))
      })
    ...
    updateRows () {
      this.$selectItem.each((i, el) => {
        this.data[$(el).data('index_bak.html')][this.header.stateField] = $(el).prop('checked')
      })
    }
    ...
    insertRow (params) {
      if (!params.hasOwnProperty('index_bak.html') || !params.hasOwnProperty('row')) {
        return
      }
      this.options.data.splice(params.index_bak, 0, params.row)
      this.initSearch()
      this.initPagination()
      this.initSort()
      this.initBody(true)
    ...
    updateRow (params) {
      ...
      for (const params of allParams) {
        ...
        $.extend(this.options.data[params.index_bak], params.row)
      }
    ...
    toggleRow (params, visible) {
      ...
      if (params.hasOwnProperty('index_bak.html')) {
        row = this.getData()[params.index_bak]
    ...
    updateCell (params) {
      ...
      this.data[params.index_bak][params.field] = params.value
    ...
    expandAllRows (isSubTable) {
      if (isSubTable) {
        ...
        for (let i = 0; i < trs.length; i++) {
          this.expandRow_(true, $(trs[i]).data('index_bak.html'))
        }
      }
    }
    ...
    collapseAllRows (isSubTable) {
      if (isSubTable) {
        ...
        const trs = this.$body.children()
        for (let i = 0; i < trs.length; i++) {
          this.expandRow_(false, $(trs[i]).data('index_bak.html'))
        }
      }
    }
